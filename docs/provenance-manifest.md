...
**Measured 2026-09-20 at `aa5f1984`. Regenerate with `scripts/provenance_manifest.sh`.**
...
At `aa5f1984`, **1233 commits**, spanning **2026-03-15 to 2026-09-20**.

| | Commits |
|---|---:|
| Total reachable | 1233 |
| Merge commits (`dco.yml` does not examine these) | 125 |
| **Non-merge commits — the population `dco.yml` grades** | **1108** |

Git author field, all 1233 commits:

| Author | Commits |
|---|---:|
| `Tom F. <tomf@tomtomtech.net>` | 572 |
...
Applying `dco.yml`'s rules to all 1108 non-merge commits:

| Outcome | Commits | Share |
|---|---:|---:|
| **Would pass** — human author, matching `Signed-off-by` | **569** | 51.4% |
| Fail — author `noreply@anthropic.com` | 477 | 43.1% |
| Fail — author `github-actions[bot]` | 46 | 4.2% |
...
126 to 569, and at this measurement the compliant commits become the **majority**
...
`v0.7.0`) is where `PROVENANCE.md` §3.2 took effect. Of the **592 non-merge
commits since** — the population `dco.yml` grades — **zero** are AI-authored:

| Author, `b416c1a..aa5f1984` | Commits |
|---|---:|
| `Tom F. <tomf@tomtomtech.net>` | 572 |
...
| | |
|---|---|
| Commits whose SHA changes | **1233 — all of them** |
