...
**Measured 2026-09-10 at `05668a5`. Regenerate with `scripts/provenance_manifest.sh`.**
...
> passes the project's own DCO gate is **46.3%** (41.4% at 0.11.0, 19.4% at
...
At `05668a5`, **1102 commits**, spanning **2026-03-15 to 2026-09-10**.

| | Commits |
|---|---:|
| Total reachable | 1102 |
| Merge commits (`dco.yml` does not examine these) | 112 |
| **Non-merge commits — the population `dco.yml` grades** | **990** |

Git author field, all 1102 commits:

| Author | Commits |
|---|---:|
| `Claude <noreply@anthropic.com>` | 478 |
| `Tom F. <tomf@tomtomtech.net>` | 459 |
...
Applying `dco.yml`'s rules to all 990 non-merge commits:

| Outcome | Commits | Share |
|---|---:|---:|
| **Would pass** — human author, matching `Signed-off-by` | **458** | 46.3% |
| Fail — author `noreply@anthropic.com` | 477 | 48.2% |
...
`v0.7.0`) is where `PROVENANCE.md` §3.2 took effect. Of the **494 commits since**,
**zero** are AI-authored:

| Author, `b416c1a..05668a5` | Commits |
|---|---:|
| `Tom F. <tomf@tomtomtech.net>` | 459 |
...
Amending it changes its SHA, and therefore the SHA of all 1101 descendants.

| | |
|---|---|
| Commits whose SHA changes | **1102 — all of them** |
