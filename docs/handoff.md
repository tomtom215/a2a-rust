...
2. **Release PR — `release/v0.12.1`.** Its content is **already written** and
   pushed. When opening it, end the description with the exact line
   `_Generated by [Claude Code](<session url>)_` under a `---` rule: the
   harness appends that footer unless the body already ends with it, matching
   on that form only, so a body ending with the other documented variant
   (`🤖 Generated with ...`) gets a second footer stacked on. #131 was opened
   that way and had to be corrected. `RELEASING.md` §1 says to cut this branch from `main`; it is instead
