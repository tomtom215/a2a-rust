...
> 2026-07-24; the project is now four releases on. The current figures live
> in [`docs/provenance-manifest.md`](provenance-manifest.md), which is
> regenerated by `scripts/provenance_manifest.sh` and gated at release time,
> and in [`CHANGELOG.md`](../CHANGELOG.md), [`STABILITY.md`](../STABILITY.md)
> and [`docs/lf-readiness-review.md`](lf-readiness-review.md). The `a2a-rs`
> figures are as of `94f4d32`, 2026-07-27, and have not been re-checked
> since. The comparison's *reasoning* — which capabilities exist where, and
> the provenance question in Section 5 — is what remains worth reading; the
> numbers are history.
...
| OpenTelemetry / OTLP export | ❌ (no `opentelemetry` dependency anywhere in the workspace) | ◑ (`otel` feature: **metrics only** — no `TracerProvider` and no span export. W3C trace **context** is propagated as of 0.13: the server parses an inbound `traceparent` and the client writes one over JSON-RPC, REST and gRPC, so a delegation chain shares one trace id, but nothing records a span. The `websocket` transport carries no `traceparent` and cannot — it has headers only on the HTTP upgrade, and W3C §3.4 forbids the connect-time value that would be the alternative. Corrected twice: this row read "traces + metrics" until 2026-09-19, which overclaimed, then "no `traceparent` anywhere", which went stale the moment propagation shipped) |
