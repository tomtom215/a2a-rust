<!--
  ...
  refreshed. Upstream amended the document in place under the same `1.0.0`
  version string, so the two disagreed while both claimed to be v1.0.0 — in
  §5.4's error table (six of nine rows), in type names, and in the push
  notification Content-Type, among others. This repository's SDK and its TCK
  both implemented the stale table, so they agreed with each other and failed
  a conformant third-party agent. Re-read this file against upstream before
