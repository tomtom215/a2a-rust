...
| Stop sending the legacy bare `a2a-notification-token` header | **Done** — two sites, not one: `HttpPushSender` no longer *sends* it (the row's actual wording), and it is dropped from the default CORS `allow_headers`. Canonical `x-a2a-notification-token` retained, and the `push_sender_https_e2e` assertion inverted to pin the header's absence rather than deleted. **Reversed for the sender on 2026-09-22:** the "bare" name is a2a-go v2.5.0's `A2A-Notification-Token` (`a2asrv/push/sender.go`), so a Go webhook read no token from this sender; `HttpPushSender` now sends both spellings and `push::webhook::notification_token` reads either. The CORS default is unchanged |
