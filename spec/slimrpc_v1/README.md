...
| branch | file | status |
|---|---|---|
| `feat/slimrpc-collaborative-channel` | `spec/v1/a2a-broadcast-live.md` (since 2026-09-10, `66ee9e0`: the transport-independent half of broadcast-live, split out of the SLIMRPC profile below) | **not implemented here.** An A2A 1.1 extension: its relay model can run over 1.0 `SendMessage`/`SendStreamingMessage`, but its semantics are 1.1's timeline (`TimelineEntry`) and its native model requires `SendLiveMessage`; neither exists in any released A2A specification. Same decision as the profile below; re-triage when A2A 1.1 ships. Triaged in `scripts/check_slimrpc_spec.sh`. |
| `feat/slimrpc-collaborative-channel` | `spec/v1/a2a-shared-task.md` (since 2026-09-10, `40c2360`: the shared-task extension the broadcast-live design builds on) | **not implemented here.** Transport-independent — an extension URI (`https://a2a-protocol.org/extensions/shared-task/v1`), a `task-sender` metadata key and multi-sender rules, nothing SLIMRPC-specific — so it is not this binding's to implement; its §3.3 preserves the sender on 1.1's `TimelineEntry`, which no released A2A specification defines. Re-triage with `a2a-broadcast-live.md` when A2A 1.1 ships. Triaged in `scripts/check_slimrpc_spec.sh`. |
