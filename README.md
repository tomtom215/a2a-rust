...
| **TLS support** | HTTPS via `rustls`, no OpenSSL dependency — on by default in the client/SDK (`tls-rustls`; `default-features = false` opts the client out, but not yet the SDK — audit K1), and the server's push sender delivers to HTTPS webhooks with it |
