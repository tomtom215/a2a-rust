...
| **Graceful shutdown** | In the order that ends work instead of orphaning it, and reported rather than assumed. `Server::serve_with_shutdown()` stops accepting, cancels every in-flight task and waits up to `task_grace` for the executors to cancel what they delegated and write a terminal event (`RequestHandler::cancel_in_flight()`, callable on its own behind Axum), then drains connections; its `ServeReport` names any task that ignored cancellation and any connection abandoned at the deadline. `RequestHandler::shutdown()` then runs the executor's cleanup hook, and its `ShutdownReport` counts any live stream it had to cut |
