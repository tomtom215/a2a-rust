...
//!
//! # Why a delegating example measures its own surface
//!
//! The interesting part here is the fan-out to worker agents in four other
//! languages. The coordinator is nonetheless an A2A *server* in its own right,
//! and its surface is the part a user of this SDK depends on — so it is worth
//! measuring even when no worker is up.
//!
//! Before 2026-08-11 this example served one binding of four and drove one
//! method; it sent a single message and waited for `Ctrl+C`.
//!
//! Worker reachability is probed and reported separately, so "the coordinator
//! answered" is never quietly upgraded to "four languages round-tripped".
...
use crate::{CoordinatorExecutor, Worker, make_coordinator_card};
...
pub async fn start(reachable: Vec<&'static Worker>) -> Result<Endpoints, BoxErr> {
    ...
    let mut card = make_coordinator_card(&url);
    card.supported_interfaces = interfaces(&ep);

    let handler: Arc<RequestHandler> = Arc::new(
        RequestHandlerBuilder::new(CoordinatorExecutor { reachable })
...
pub async fn start_restricted() -> Result<String, BoxErr> {
    let (listener, addr) = bind().await?;
    let url = format!("http://{addr}");
    let mut card = make_coordinator_card(&url);
    card.name = "Restricted Coordinator".into();
