...
mod tests {
    ...
    // `bom_in_first_line_buf_stripped_by_process_line` was removed 2026-08-19.
    //
    // It was 47 lines, every one of them a comment, ending in "Skip this test".
    // Its body constructed a parser, bound it to `_p`, and did nothing else, so
    // it could not fail — while its name asserted that a fragmented BOM is
    // stripped by `process_line`, which is a real and load-bearing claim.
    //
    // Its reasoning was also out of date: it argued from a feed-time BOM
    // fast-path that has since been removed, so its conclusion ("lines 165-168
    // are dead code in practice") described code that no longer exists.
    // `process_line` is now the *only* place a BOM is stripped.
    //
    // The behaviour it named is covered, with assertions, by
    // `bom_split_one_byte_at_a_time_first_event_survives` and
    // `bom_split_two_then_one_byte_first_event_survives` above.
